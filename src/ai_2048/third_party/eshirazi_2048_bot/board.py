...
class Board:
    ...
    def clear(self):
        self.b = [[0 for _ in irange(BOARD_SIZE)] for _ in irange(BOARD_SIZE)]
    ...
    def __init__(
        ...
        if init_board is None:
            ...
            initializers = list(random.choice([(2, 4), (2, 2)]))
            ...
            # Copy constructor
            self.b = [
                [init_board[y, x] for x in irange(BOARD_SIZE)]
    ...
    def move_only_swipe(self, move):
        ...
        adapt = lambda i: i if direction != 1 else (BOARD_SIZE - i - 1)
    ...
    def add_random_tile(self):
        try:
            self[random.choice(tuple(self.get_free_tiles()))] = random.choice(
                POSSIBLE_NEW_TILES
            )
        except IndexError:
            raise IllegalMoveException()
